...
import pandas as pd
import numpy as np
import altair as alt  # Já vem instalado junto com o Streamlit por padrão
...
with aba1:
    ...
    st.markdown(f"### Perfil de Filtração - Linha de Tendência para {pressao_nominal} mca")
    
    # Adicionando a predição para plotagem
    df_valido['Linha de Tendência'] = y_pred

    # 🛠️ CONSTRUÇÃO DO GRÁFICO AVANÇADO COM ALTAIR
    # Camada 1: Os pontos experimentais originais isolados em azul
    pontos = alt.Chart(df_valido).mark_circle(size=70, color='#1f77b4').encode(
        x=alt.X('V (m³):Q', title='V (m³)'),
        y=alt.Y('tempo por unidade de volume:Q', title='tempo por unidade de volume')
    )

    # Camada 2: A reta de tendência pontilhada (strokeDash) em uma cor cinza suave discreta (#94a3b8)
    reta_tendencia = alt.Chart(df_valido).mark_line(
        color='#94a3b8', 
        strokeDash=[4, 4], 
        strokeWidth=1.5
    ).encode(
        x='V (m³):Q',
        y='Linha de Tendência:Q'
    )

    # Sobrepondo as duas camadas e plotando na tela
    # Correção da linha 105: troque 'recta_tendencia' por 'reta_tendencia'
    grafico_final = alt.layer(pontos, reta_tendencia).properties(height=400).interactive()
...
with aba2:
    ...
    # Controles teóricos agrupados em colunas
    ...
    # Conversão de mca para Pascal (Unidade SI necessária para o manual: 1 mca ≈ 9806.65 Pa)
    delta_p_pa = p_teorica_mca * 9806.65

    # Cálculo dos parâmetros 'a' e 'b' teóricos de Svarovsky
